...
function renderTrail(value) {
  ...
  for (let i = 3; i < 10; i += 2) {
    if (value.length > i) {
      trail.push(<div className="circle" style={{ top: value[value.length - i].pos.y, left: value[value.length - i].pos.x, background: `#${i}${i}${i}`}} />);
    }
  }
  return trail;
}
  ...
  return (
    <div className="grid">
      <div className="CircleContainer">
        {renderTrail(value)}
        {value.length > 0 ? (
          <div className="circle" style={{ top: value[value.length - 1].pos.y, left: value[value.length - 1].pos.x }} />
        ) : null}
      </div>
      <img
        onMouseMove={(e) => {
